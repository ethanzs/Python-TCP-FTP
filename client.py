...
def ftp_get(command):
    if(ack[0] == 'D'):  # if file is found on server side
        ...
        print("'{fileName}' successfully downloaded.".format(
            fileName=fileName))
    else:
        print(ack)
...
def ftp_put(command):
    if(len(command) != 3):
        ...
        directory = fileNameFull[:index+1] # use reverse search index to assign fileName directory
        found = False
        try: 
        ...
        if(found): # if file was found in directory specified
            ...
            print("'{fileName}' was successfully uploaded.".format(
                fileName=fileName))
        else:
            send_data(sc, "Not found")
            print("File '{fileName}' was not found in this directory.".format(
                fileName=fileName))
    else:
