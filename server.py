...
def ftp_put(command):  # download a file from client
    ...
    if ack[0] == "D":  # if file was found
        ...
        sizeReceived = 0
        while (
            sizeReceived != fileSize
        ):  # receive data until size received matches fileSize
        ...
        print("'{fileName}' was successfully downloaded.".format(fileName=fileName))
    else:
        print(ack)
...
while True:
    ...
    print(
        "Connection from '{address_c}' has been established on port {C_PORT}.".format(
            address_c=address_c, C_PORT=C_PORT
        )
    )
    print(
        "Connection from '{address_d}' has been established on port {D_PORT}".format(
    ...
    while command != "exit":
        ...
        print("{address_c[0]}: {command}".format(address_c=address_c, command=command))
        ack = ""
        cwd = os.getcwd()
        # decode command
    ...
    print(
        "{address_c[0]} has terminated its connection on port {C_PORT}.".format(
            address_c=address_c, C_PORT=C_PORT
        )
    )
    print(
        "{address_d[0]} has terminated its connection on port {D_PORT}".format(
            address_d=address_d, D_PORT=D_PORT
        )
    )
